...
import {TelemetryInfo} from './internal';
...
export class TelemetryInfoImpl implements TelemetryInfo, TypeChecker {
  ...
  constructor(dtdlVersion: number, id: string, childOf: string|undefined, definedIn: string|undefined, entityKind: EntityKind = EntityKind.TELEMETRY) {
...
.add('dtmi:dtdl:class:Telemetry');
    ...
    this._badTypeActionFormat[2] = `Provide a value for property '{property}' with @type Telemetry.`;
    this._badTypeActionFormat[3] = `Provide a value for property '{property}' with @type Telemetry.`;
    this._badTypeCauseFormat[2] = `{primaryId:p} property '{property}' has value{secondaryId:e} that does not have @type of Telemetry.`;
    this._badTypeCauseFormat[3] = `{primaryId:p} property '{property}' has value{secondaryId:e} that does not have @type of Telemetry.`;
  }
  ...
  doesHaveType(typeId: string): boolean {
    return TelemetryInfoImpl._versionlessTypes.has(new InDTMI(typeId).versionless) ||
this.supplementalTypes.some((st) => (st as SupplementalTypeInfoImpl).doesHaveType(typeId))
  ...
  static parseObject(model: Model, objectPropertyInfoList: ParsedObjectPropertyInfo[], elementPropertyConstraints: ElementPropertyConstraint[], valueConstraints: ValueConstraint[], aggregateContext: AggregateContext, parsingErrors: ParsingError[], object: {[index: string]: any}, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, dtmiSeg: string|undefined, keyProp: string|undefined, idRequired: boolean, typeRequired: boolean, allowIdReferenceSyntax: boolean, allowedVersions: Set<number>): any {
    // This is a method to parse the object read from DTDL into a type of TelemetryInfo
    ...
    (elementInfo as TelemetryInfoImpl).sourceObject = object;
    switch (childAggregateContext.dtdlVersion) {
    case 2: {
      (elementInfo as TelemetryInfoImpl)?.parsePropertiesV2(model, objectPropertyInfoList, elementPropertyConstraints, childAggregateContext, parsingErrors, object, definedIn, allowIdReferenceSyntax);
      break;
    }

    case 3: {
      (elementInfo as TelemetryInfoImpl)?.parsePropertiesV3(model, objectPropertyInfoList, elementPropertyConstraints, childAggregateContext, parsingErrors, object, definedIn, allowIdReferenceSyntax);
      break;
    }
    }
  ...
  static parseTypeArray(tokenArr: any[], elementId: string, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, aggregateContext: AggregateContext, parsingErrors: ParsingError[]): TelemetryInfo|undefined {
    const materialKinds : EntityKind[] = [];
    const elementInfo : Reference<TelemetryInfo> = referenceInit();
    ...
    for (const supplementalTypeId of supplementalTypeIds) {
      ...
      if (elementInfo.ref !== undefined && elementInfo.ref.entityKind !== undefined) {
        if (!(supplementalTypeInfo as SupplementalTypeInfoImpl)?.allowedCotypeKinds.includes(elementInfo.ref.entityKind)) {
          ...
          (elementInfo.ref as TelemetryInfoImpl).addType(supplementalTypeId, supplementalTypeInfo);
        }
      }
    }
  ...
  static tryParseTypeStringV2(typestring: string, elementId: string, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, materialKinds: EntityKind[], supplementalTypeIds: string[], elementInfo: Reference<TelemetryInfo>, undefinedTypes: string[], aggregateContext: AggregateContext, parsingErrors: ParsingError[]): boolean {
    switch (typestring) {
    case 'Telemetry':
  ...
  parsePropertiesV2(model: Model, objectPropertyInfoList: ParsedObjectPropertyInfo[], elementPropertyConstraints: ElementPropertyConstraint[], aggregateContext: AggregateContext, parsingErrors: ParsingError[], object: {[index: string]: any}, definedIn: string|undefined, allowIdReferenceSyntax: boolean): void {
    ...
    for (const propKey in object) {
      ...
      case 'dtmi:dtdl:property:schema;2':
        ...
        valueCount = SchemaInfoImpl.parseToken(model, objectPropertyInfoList, elementPropertyConstraints, this._schemaValueConstraints, aggregateContext, parsingErrors, propValue, this.id, definedIn ?? this.id, 'schema', undefined, undefined, false, true, allowIdReferenceSyntax, this._schemaAllowedVersionsV2);
        if (valueCount < 1) {
          parsingErrors.push(new ParsingError(
  ...
  parsePropertiesV3(model: Model, objectPropertyInfoList: ParsedObjectPropertyInfo[], elementPropertyConstraints: ElementPropertyConstraint[], aggregateContext: AggregateContext, parsingErrors: ParsingError[], object: {[index: string]: any}, definedIn: string|undefined, allowIdReferenceSyntax: boolean): void {
    ...
    for (const propKey in object) {
      ...
      case 'dtmi:dtdl:property:schema;3':
        schemaPropertyMissing = false;
        valueCount = SchemaInfoImpl.parseToken(model, objectPropertyInfoList, elementPropertyConstraints, this._schemaValueConstraints, aggregateContext, parsingErrors, propValue, this.id, definedIn ?? this.id, 'schema', undefined, undefined, false, true, allowIdReferenceSyntax, this._schemaAllowedVersionsV3);
        if (valueCount < 1) {
          parsingErrors.push(new ParsingError(
            'dtmi:dtdl:parsingError:propertyCountBelowMin',
            {
...
**/
  setPartitionInfo(partitionJsonText: string): void {
    throw new Error(`attempt to set partition info on non-partition type TelemetryInfoInfo`);
  }
  ...
  checkRestrictionsV2(parsingErrors: ParsingError[]) {
    if (this._schemaInstanceProperties !== undefined) {
      for (const instanceProp of this._schemaInstanceProperties) {
        if (!(this.schema as SchemaInfoImpl)?.validateInstanceElement(this.supplementalProperties[instanceProp])) {
          parsingErrors.push(new ParsingError(
...
**/
  checkDepthOfElementSchemaOrSchema(depth: number, depthLimit: number, tooDeepElementId: Reference<InDTMI>, parsingErrors: ParsingError[]): boolean {
    ...
    if (this.schema !== undefined) {
      if (!(this.schema as SchemaInfoImpl).checkDepthOfElementSchemaOrSchema(depth + 1, depthLimit, tooDeepElementId, parsingErrors)) {
  ...
  tryGetDescendantSchemaOrContentsComponentNarrow(elementId: Reference<InDTMI>): boolean {
    ...
    if (this.schema !== undefined) {
      if ((this.schema as SchemaInfoImpl).entityKind === EntityKind.COMPONENT) {
        elementId.ref = new InDTMI((this.schema as SchemaInfoImpl).id);
        this._idOfDescendantSchemaOrContentsComponentNarrow = elementId.ref;
        return true;
      }

      if ((this.schema as SchemaInfoImpl).tryGetDescendantSchemaOrContentsComponentNarrow(elementId)) {
        this._idOfDescendantSchemaOrContentsComponentNarrow = elementId.ref;
        return true;
      }
    }
  ...
  checkDescendantEnumValueDataType(ancestorId: InDTMI, datatype: string, parsingErrors: ParsingError[]): void {
    ...
    if (this.schema !== undefined) {
      (this.schema as SchemaInfoImpl).checkDescendantEnumValueDataType(ancestorId, datatype, parsingErrors);
    }
  }
  ...
  tryGetDescendantSchemaArray(elementId: Reference<InDTMI>): boolean {
    ...
    if (this.schema !== undefined) {
      if ((this.schema as SchemaInfoImpl).entityKind === EntityKind.ARRAY) {
        elementId.ref = new InDTMI((this.schema as SchemaInfoImpl).id);
        this._idOfDescendantSchemaArray = elementId.ref;
        return true;
      }

      if ((this.schema as SchemaInfoImpl).tryGetDescendantSchemaArray(elementId)) {
        this._idOfDescendantSchemaArray = elementId.ref;
        return true;
      }
    }
  ...
  getCountOfContentsOrFieldsOrEnumValuesOrRequestOrResponseOrPropertiesOrSchemaOrElementSchemaOrMapValueNarrow(parsingErrors: ParsingError[]): number {
    ...
    if (this.schema !== undefined) {
      this._countOfContentsOrFieldsOrEnumValuesOrRequestOrResponseOrPropertiesOrSchemaOrElementSchemaOrMapValueNarrowValue += (this.schema as SchemaInfoImpl).getCountOfContentsOrFieldsOrEnumValuesOrRequestOrResponseOrPropertiesOrSchemaOrElementSchemaOrMapValueNarrow(parsingErrors) + 1;
    }
