...
export class TimeInfoImpl implements TimeInfo, TypeChecker {
  ...
  private _elementValueInstanceRegexPatternV2: RegExp = /^[0-9]{2}:[0-9]{2}:[0-9]{2}(?:\.[0-9]+)?(?:Z|(?:\+|-)[0-9]{2}:[0-9]{2})$/;
  private _elementValueInstanceRegexPatternV3: RegExp = /^[0-9]{2}:[0-9]{2}:[0-9]{2}(?:\.[0-9]+)?(?:Z|(?:\+|-)[0-9]{2}:[0-9]{2})$/;
  ...
  constructor(dtdlVersion: number, id: string, childOf: string|undefined, definedIn: string|undefined, entityKind: EntityKind = EntityKind.TIME) {
...
.add('dtmi:dtdl:class:Time');
    ...
    this._badTypeActionFormat[2] = `Choose the following value for property '{property}': time.`;
    this._badTypeActionFormat[3] = `Choose the following value for property '{property}': time.`;
  ...
  doesHaveType(typeId: string): boolean {
    return TimeInfoImpl._versionlessTypes.has(new InDTMI(typeId).versionless) ||
this.supplementalTypes.some((st) => (st as SupplementalTypeInfoImpl).doesHaveType(typeId))
  ...
  static parseObject(model: Model, objectPropertyInfoList: ParsedObjectPropertyInfo[], elementPropertyConstraints: ElementPropertyConstraint[], valueConstraints: ValueConstraint[], aggregateContext: AggregateContext, parsingErrors: ParsingError[], object: {[index: string]: any}, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, dtmiSeg: string|undefined, keyProp: string|undefined, idRequired: boolean, typeRequired: boolean, allowIdReferenceSyntax: boolean, allowedVersions: Set<number>): any {
    // This is a method to parse the object read from DTDL into a type of TimeInfo
    ...
    (elementInfo as TimeInfoImpl).sourceObject = object;
    switch (childAggregateContext.dtdlVersion) {
    case 2: {
      (elementInfo as TimeInfoImpl)?.parsePropertiesV2(model, objectPropertyInfoList, elementPropertyConstraints, childAggregateContext, parsingErrors, object, definedIn, allowIdReferenceSyntax);
      break;
    }

    case 3: {
      (elementInfo as TimeInfoImpl)?.parsePropertiesV3(model, objectPropertyInfoList, elementPropertyConstraints, childAggregateContext, parsingErrors, object, definedIn, allowIdReferenceSyntax);
      break;
    }
    }
  ...
  static parseTypeArray(tokenArr: any[], elementId: string, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, aggregateContext: AggregateContext, parsingErrors: ParsingError[]): TimeInfo|undefined {
    const materialKinds : EntityKind[] = [];
    const elementInfo : Reference<TimeInfo> = referenceInit();
    ...
    for (const supplementalTypeId of supplementalTypeIds) {
      ...
      if (elementInfo.ref !== undefined && elementInfo.ref.entityKind !== undefined) {
        if (!(supplementalTypeInfo as SupplementalTypeInfoImpl)?.allowedCotypeKinds.includes(elementInfo.ref.entityKind)) {
          ...
          (elementInfo.ref as TimeInfoImpl).addType(supplementalTypeId, supplementalTypeInfo);
        }
      }
    }
  ...
  static tryParseTypeStringV2(typestring: string, elementId: string, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, materialKinds: EntityKind[], supplementalTypeIds: string[], elementInfo: Reference<TimeInfo>, undefinedTypes: string[], aggregateContext: AggregateContext, parsingErrors: ParsingError[]): boolean {
    switch (typestring) {
    case 'Time':
...
**/
  setPartitionInfo(partitionJsonText: string): void {
    throw new Error(`attempt to set partition info on non-partition type TimeInfoInfo`);
  }
