...
export class ResultFormatter {
  ...
  public install(key: string, value: string) {
    ...
    for (const expansionTypeKey in Expansion) { // eslint-disable-line guard-for-in
  ...
  private _expandValue(value: string, expansionType: ExpansionType): string {
    ...
    case 'Possessive':
      if (dtmi.labels[dtmi.labels.length - 1].startsWith('__')) {
        return this._buildNominativePhrase(dtmi, dtmi.labels.length - 2) + ` has '${dtmi.labels[dtmi.labels.length - 2].substr(1)}' value with name '${dtmi.labels[dtmi.labels.length - 1].substr(2)}' whose`;
      } else {
        return this._buildNominativePhrase(dtmi, dtmi.labels.length - 1) + ` has '${dtmi.labels[dtmi.labels.length - 1].substr(1)}' value whose`;
      }
    case 'Relative':
      if (dtmi.labels[dtmi.labels.length - 1].startsWith('__')) {
  ...
  private _getBasicExpansion(value: string, expansionType: ExpansionType, defaultValue: string): string {
    switch (expansionType) {
    case 'Nominative':
      return value;
    case 'Possessive':
  ...
  private _buildNominativePhrase(dtmi: InDTMI, labelCount: number): string {
    if (dtmi.labels[labelCount - 1].startsWith('__')) {
      return this._buildNominativePhrase(dtmi, labelCount - 2) + ` has '${dtmi.labels[labelCount - 2].substr(1)}' value with name '${dtmi.labels[labelCount - 1].substr(2)}' which`;
    } else if (dtmi.labels[labelCount - 1].startsWith('_')) {
      return this._buildNominativePhrase(dtmi, labelCount - 1) + ` has '${dtmi.labels[labelCount - 1].substr(1)}' value which`;
    } else {
      let ancestorId = 'dtmi';
