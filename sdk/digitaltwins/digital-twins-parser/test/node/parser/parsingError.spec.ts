...
describe('Test ParsingError', function() {
  it('sets properties and formats strings', function() {
    const parsingError = createParsingError(
      ...
      {
        cause: '{primaryId:p} property {property} has value {value}; however, {secondaryId:n} specifies a different datatype.',
    ...
    expect(parsingError.cause).to.equal(`dtmi:test:myInterface;1 has 'contents' value with name 'myTelemetry' which has 'schema' value whose property elementSchema has value 42; however, dtmi:test:myInterface;1 has 'contents' value with name 'myProperty' which has 'annotation' value which specifies a different datatype.`);
    expect(parsingError.action).to.equal(`Change the value of property elementSchema to a value with a matching datatype.`);
    expect(parsingError.primaryId).to.equal('dtmi:test:myInterface:_contents:__myTelemetry:_schema;1');
    expect(parsingError.secondaryId).to.equal('dtmi:test:myInterface:_contents:__myProperty:_annotation;1');
  ...
  it('allows undefined properties', function() {
    const parsingError = createParsingError(
      'dtmi:dtdl:parsingError:unhelpfulError',
      {
