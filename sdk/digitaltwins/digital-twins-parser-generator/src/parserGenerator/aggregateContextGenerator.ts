...
export class AggregateContextGenerator implements TypeGenerator {
  ...
  constructor(contexts: {[x:string]: any}, dtdlVersionAllowingLocalTerms: number[], affiliateContextsImplicitDtdlVersions: {[x:string]: number}) {
  ...
  generateCode(parserLibrary: TsLibrary) {
    ...
    contextClass.import(`import {ContextHistory, InDTMI, ParsingError, createParsingError, ParsingException, VersionedContext} from '../parser';`);
    contextClass.import(`import {IdValidator} from './internal'`);
    contextClass.header.line('/* eslint-disable camelcase */');
    contextClass.docString.line('Class for parsing and storing information from JSON-LD context blocks.');
  ...
  private _generateFields(contextClass: TsClass) {
    contextClass
      .field({name: '_dtdlVersionsAllowingLocalTerms', type: 'number[]', access: TsAccess.Private, isStatic: true, value: '[0]'})
      .field({name: '_affiliateContextsImplicitDtdlVersions', type: `{[x:string]: number}`, isStatic: true, value: '{}'});
  ...
  private _generateStaticConstructor(contextClass: TsClass) {
    for (const [key, value] of Object.entries(this._affiliateContextsImplicitDtdlVersions)) {
      contextClass.staticCtor.body.line(`this._affiliateContextsImplicitDtdlVersions['${key}'] = ${value};`);
    }
    ...
    const dtdlContextMethod = contextClass.method({name: '_getDtdlContextHistory', returnType: `ContextHistory`, access: TsAccess.Private, isStatic: true});
    ...
    for (const [contextSpecifier, termDefinitions] of Object.entries(this._contexts)) {
      if (contextSpecifier.startsWith(ParserGeneratorValues.DtdlContextPrefix)) {
        ...
        if (affiliateIndex === undefined) {
          ...
          const affiliateContextMethod = contextClass.method({name: `_getAffiliate${affiliateIndex}ContextHistory`, returnType: `ContextHistory`, access: TsAccess.Private, isStatic: true});
        ...
        this._addContextVersion(affiliateContextMethods[affiliateIndex].body, contextSpecifier, termDefinitions);
      }
    }
    ...
    for (const [key, value] of Object.entries(affiliateIndices)) {
      contextClass.staticCtor.body.line(`AggregateContext._affiliateContextHistories['${key}'] = AggregateContext._getAffiliate${value}ContextHistory();`);
    }
  }

  private _addContextVersion(contextMethodBody: TsScope, contextSpecifier: string, termDefinitions: {[x:string]: string}) {
    ...
    contextMethodBody.line(`const ${contextVar} = new VersionedContext(${majorVersion}, ${minorVersion});`);

    for (const [key, value] of Object.entries(termDefinitions)) {
      contextMethodBody.line(`${contextVar}.addDefinition('${key}', new ${ParserGeneratorValues.IdentifierType}('${value}'));`);
    }
