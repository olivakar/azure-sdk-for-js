...
export class ExtensibleMaterialClass {
  ...
  public addCaseToParseTypeStringSwitch(obverseClass: TsClass, switchOnExtensionKind:TsScope, extensibleMaterialSubtypes:string[], parentIdVar:string, definedInVar:string) : void {
    ...
    if (extensibleMaterialSubtypes.includes(this._typeName)) {
      ...
      switchOnExtensionKind
        .line(`elementInfo.ref = new ${this._className}(${this._dtdlVersion}, elementId, ${parentIdVar}, ${definedInVar});`)
        .line(`(elementInfo.ref as ${this._className}).addType(supplementalTypeId.value, supplementalTypeInfo);`)
        .line(`materialKinds.push(${this._kindEnum}.${this._kindValue});`)
        .line(`return true;`);
    } else {
      switchOnExtensionKind
