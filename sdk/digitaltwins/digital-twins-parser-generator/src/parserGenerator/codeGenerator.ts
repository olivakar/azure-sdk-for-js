...
export class CodeGenerator {
  public static execute(inputDigest:string, outputDirectory:string, dtdlVersion:string) {
    fs.readFile(inputDigest, (err:Error|null, rawMetamodelDigest:Buffer) => {
      ...
      typeGenerators.push(new AggregateContextGenerator(parsedMetamodelDigest.contexts, parsedMetamodelDigest.dtdlVersionsAllowingLocalTerms, parsedMetamodelDigest.affiliateContextsImplicitDtdlVersions));
      typeGenerators.push(new HelpersGenerator(baseClassName));
      typeGenerators.push(new StandardElementsGenerator(baseClassName, parsedMetamodelDigest.elements));
      ...
      typeGenerators.push(new ModelGenerator(baseClassName, parsedMetamodelDigest.partitionClasses, parsedMetamodelDigest.partitionRestrictions));
      ...
      typeGenerators.push(new PartitionTypeCollectionGenerator(parsedMetamodelDigest.partitionClasses));
      typeGenerators.push(new RootableTypeCollectionGenerator(parsedMetamodelDigest['rootableClasses']));
      typeGenerators.push(new IdValidatorGenerator(parsedMetamodelDigest['identifierDefinition'], parsedMetamodelDigest['identifierReference']));
      typeGenerators.push(new MaterialTypeNameCollectionGenerator(Object.keys(parsedMetamodelDigest['materialClasses']), Object.values(parsedMetamodelDigest['contexts'])));
      ...
      typeGenerators.push(new SupplementalTypeCollectionGenerator(parsedMetamodelDigest['supplementalTypes'], parsedMetamodelDigest['contexts'], parsedMetamodelDigest['extensibleMaterialClasses'], baseClassName));
      // typeGenerators.push(new ModelExemplifierGenerator(baseClassName)); // ONLY USED BY .NET FOR TEST CASE GENERATION
      // typeGenerators.push(new TypeExclusionGenerator(baseClassName)); // ONLY USED BY .NET FOR TEST CASE GENERATION
  ...
  private static _generateMaterialClasses(metamodelDigest: MetamodelDigest): TypeGenerator[] {
    ...
    for (const [key, value] of Object.entries(metamodelDigest.extensibleMaterialClasses)) {
      ...
      for (const extensibleMaterialClassName of value) {
        ...
        extensibleMaterialClasses[key].push(new ExtensibleMaterialClass(key as unknown as number, extensibleMaterialClassName, baseKindEnum));
      }
    }
    ...
    typeGenerators.push(new MaterialClassGenerator({
