...
import {ObjectInfo} from './internal';
...
export class ObjectInfoImpl implements ObjectInfo, TypeChecker {
  ...
  private _fieldsValueConstraints: ValueConstraint[] = [];
  private _fieldsInstanceProperties: string[] = [];
  private _fieldsAllowedVersionsV2: Set<number> = new Set<number>().add(2);
  private _fieldsAllowedVersionsV3: Set<number> = new Set<number>().add(3);
  ...
  constructor(dtdlVersion: number, id: string, childOf: string|undefined, definedIn: string|undefined, entityKind: EntityKind = EntityKind.OBJECT) {
...
.add('dtmi:dtdl:class:Schema');
    ...
    this._badTypeActionFormat[2] = `Provide a value for property '{property}' with @type Object.`;
    this._badTypeActionFormat[3] = `Provide a value for property '{property}' with @type Object.`;
    this._badTypeCauseFormat[2] = `{primaryId:p} property '{property}' has value{secondaryId:e} that does not have @type of Object.`;
    this._badTypeCauseFormat[3] = `{primaryId:p} property '{property}' has value{secondaryId:e} that does not have @type of Object.`;
  }
  ...
  doesHaveType(typeId: string): boolean {
    return ObjectInfoImpl._versionlessTypes.has(new InDTMI(typeId).versionless) ||
this.supplementalTypes.some((st) => (st as SupplementalTypeInfoImpl).doesHaveType(typeId))
  ...
  static parseObject(model: Model, objectPropertyInfoList: ParsedObjectPropertyInfo[], elementPropertyConstraints: ElementPropertyConstraint[], valueConstraints: ValueConstraint[], aggregateContext: AggregateContext, parsingErrors: ParsingError[], object: {[index: string]: any}, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, dtmiSeg: string|undefined, keyProp: string|undefined, idRequired: boolean, typeRequired: boolean, allowIdReferenceSyntax: boolean, allowedVersions: Set<number>): any {
    // This is a method to parse the object read from DTDL into a type of ObjectInfo
    ...
    (elementInfo as ObjectInfoImpl).sourceObject = object;
    switch (childAggregateContext.dtdlVersion) {
    case 2: {
      (elementInfo as ObjectInfoImpl)?.parsePropertiesV2(model, objectPropertyInfoList, elementPropertyConstraints, childAggregateContext, parsingErrors, object, definedIn, allowIdReferenceSyntax);
      break;
    }

    case 3: {
      (elementInfo as ObjectInfoImpl)?.parsePropertiesV3(model, objectPropertyInfoList, elementPropertyConstraints, childAggregateContext, parsingErrors, object, definedIn, allowIdReferenceSyntax);
      break;
    }
    }
  ...
  static parseTypeArray(tokenArr: any[], elementId: string, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, aggregateContext: AggregateContext, parsingErrors: ParsingError[]): ObjectInfo|undefined {
    const materialKinds : EntityKind[] = [];
    const elementInfo : Reference<ObjectInfo> = referenceInit();
    ...
    for (const supplementalTypeId of supplementalTypeIds) {
      ...
      if (elementInfo.ref !== undefined && elementInfo.ref.entityKind !== undefined) {
        if (!(supplementalTypeInfo as SupplementalTypeInfoImpl)?.allowedCotypeKinds.includes(elementInfo.ref.entityKind)) {
          ...
          (elementInfo.ref as ObjectInfoImpl).addType(supplementalTypeId, supplementalTypeInfo);
        }
      }
    }
  ...
  parsePropertiesV2(model: Model, objectPropertyInfoList: ParsedObjectPropertyInfo[], elementPropertyConstraints: ElementPropertyConstraint[], aggregateContext: AggregateContext, parsingErrors: ParsingError[], object: {[index: string]: any}, definedIn: string|undefined, allowIdReferenceSyntax: boolean): void {
    ...
    for (const propKey in object) {
      ...
      case 'dtmi:dtdl:property:fields;2':
        valueCount = FieldInfoImpl.parseToken(model, objectPropertyInfoList, elementPropertyConstraints, this._fieldsValueConstraints, aggregateContext, parsingErrors, propValue, this.id, definedIn ?? this.id, 'fields', 'name', undefined, false, false, allowIdReferenceSyntax, this._fieldsAllowedVersionsV2);
        if (valueCount < 1) {
          parsingErrors.push(new ParsingError(
            'dtmi:dtdl:parsingError:propertyCountBelowMin',
            {
              cause: `{primaryId:p} property 'fields' has value valueCount values, but the required minimum count is 1`,
              action: `Add one or more 'fields' to the object until the minimum count is satisfied.`,
              primaryId: this.id,
              property: 'fields',
            }));
        }
        ...
        if (valueCount > 30) {
          parsingErrors.push(new ParsingError(
            ...
            {
              cause: `{primaryId:p} property 'fields' has value valueCount values, but the allowed maximum count is 30`,
              action: `Remove one or more 'fields' to the object until the maximum count is satisfied.`,
              primaryId: this.id,
              property: 'fields',
            }));
        }
  ...
  parsePropertiesV3(model: Model, objectPropertyInfoList: ParsedObjectPropertyInfo[], elementPropertyConstraints: ElementPropertyConstraint[], aggregateContext: AggregateContext, parsingErrors: ParsingError[], object: {[index: string]: any}, definedIn: string|undefined, allowIdReferenceSyntax: boolean): void {
    ...
    for (const propKey in object) {
      ...
      case 'dtmi:dtdl:property:fields;3':
        FieldInfoImpl.parseToken(model, objectPropertyInfoList, elementPropertyConstraints, this._fieldsValueConstraints, aggregateContext, parsingErrors, propValue, this.id, definedIn ?? this.id, 'fields', 'name', undefined, false, false, allowIdReferenceSyntax, this._fieldsAllowedVersionsV3);
        continue;
      }
  ...
  validateInstanceV2(instanceElt: any, instanceName: string|undefined): boolean {
    ...
    for (const [key, value] of Object.entries(instanceElt)) {
      if (!this.fields?.some((val) => (val as FieldInfoImpl).validateInstanceInternal(value, key))) {
        return false;
      }
    }
      ...
      for (const item of this.fields || []) {
        if (fieldsNameSet.has(item.name)) {
          parsingErrors.push(new ParsingError(
            'dtmi:dtdl:parsingError:nonUniquePropertyValue',
            {
              cause: '{{primaryId:n}} property \'fields\' contains more than one element whose property \'name\' has value \'{item.name}\'.',
              action: 'Change the value of property  \'name\' to a string value that is unique across all values of \'fields\'.',
              primaryId: this.id,
              property: 'fields',
              value: 'name',
            }));
...
**/
  checkDepthOfElementSchemaOrSchema(depth: number, depthLimit: number, tooDeepElementId: Reference<InDTMI>, parsingErrors: ParsingError[]): boolean {
    for (const item of this.fields || []) {
      if (!(item as FieldInfoImpl).checkDepthOfElementSchemaOrSchema(depth, depthLimit, tooDeepElementId, parsingErrors)) {
  ...
  getCountOfContentsOrFieldsOrEnumValuesOrRequestOrResponseOrPropertiesOrSchemaOrElementSchemaOrMapValueNarrow(parsingErrors: ParsingError[]): number {
    ...
    for (const item of this.fields || []) {
      this._countOfContentsOrFieldsOrEnumValuesOrRequestOrResponseOrPropertiesOrSchemaOrElementSchemaOrMapValueNarrowValue += (item as FieldInfoImpl).getCountOfContentsOrFieldsOrEnumValuesOrRequestOrResponseOrPropertiesOrSchemaOrElementSchemaOrMapValueNarrow(parsingErrors) + 1;
    }
