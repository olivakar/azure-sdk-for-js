...
import {MaterialClassDigest} from '../parserGenerator/metamodelDigest';
import {NameFormatter} from '../parserGenerator/nameFormatter';

export class TypeKindValuesHelper {
  public static generateConcreteTypesKindValues(baseTypeRawName:string, materialClassName:string, materialClassObject: MaterialClassDigest) : Set<string> {
    ...
    for (const dtdlVersion of materialClassObject.dtdlVersions) {
      ...
      if (dtdlVersion in materialClassObject.extensibleMaterialSubclasses) {
        const extensibleClasses : string[] = materialClassObject.extensibleMaterialSubclasses[dtdlVersion];
        extensibleClasses.forEach((element:string) => {
          typeKindValues.add(`'${NameFormatter.formatNameAsKindString(element)}'`);
        });
      }
    }
    if (NameFormatter.formatNameAsKindString(materialClassName) === baseTypeRawName.toLowerCase() || NameFormatter.formatNameAsKindString(materialClassName) === refKind) {
