...
export class HelpersGenerator implements TypeGenerator {
  ...
  private _generateAreListsIdEqualMethod(helpersClass: TsClass) {
    ...
    const method = helpersClass.method({name: 'areListsIdEqual', returnType: 'boolean', isStatic: true});
    method
    ...
    method.body
      .for('let i=0; i<list1.length; i++')
        .if(`list1[i].${ParserGeneratorValues.IdentifierName} !== list2[i].${ParserGeneratorValues.IdentifierName}`)
          .line('return false;');
  ...
  private _generateAreListsIdOrLiteralEqualMethod(helpersClass: TsClass) {
    const method = helpersClass.method({name: 'areListsIdOrLiteralEqual', returnType: 'boolean', isStatic: true});
    method
      .parameter({name: 'list1', type: `${this._baseClassName}[]`})
      .parameter({name: 'list2', type: `${this._baseClassName}[]`});
    ...
    const ifNoTUndefined = method.body
      .for('let i=0; i<list1.length; i++')
        .if(`list1[i].${ParserGeneratorValues.IdentifierName} !== undefined && list2[i].${ParserGeneratorValues.IdentifierName} !== undefined`);
    ifNoTUndefined
          .if(`list1[i].${ParserGeneratorValues.IdentifierName} !== list2[i].${ParserGeneratorValues.IdentifierName}`)
            .line('return false;');
    ifNoTUndefined
  ...
  private _generateAreDictionariesIdEqualMethod(helpersClass: TsClass) {
    const method = helpersClass.method({name: 'areDictioanriesIdEqual', returnType: 'boolean', isStatic: true});
    method
    ...
    method.body
      .if('Object.keys(dict1).length !== Object.keys(dict2).length')
        .line('return false;');
    method.body
      .line('const dict1Keys = Object.keys(dict1)');

    method.body
      .for('const [key, value] of Object.entries(dict1)')
        .if(`dict2[key] === undefined || dict2[key].${ParserGeneratorValues.IdentifierName} !== value.${ParserGeneratorValues.IdentifierName}`)
          .line('return false;');
  ...
  private _generateAreDictionariesIdOrLiteralEqualMethod(helpersClass: TsClass) {
    const method = helpersClass.method({name: 'areDictionariesIdOrLiteralEqual', returnType: 'boolean', isStatic: true});
    method
    ...
    method.body
      .for('const [key, value] of Object.entries(dict1)')
        ...
        .if('!val2')
          .line('return false;')
        .elseIf(`!value.${ParserGeneratorValues.IdentifierName} || !val2.${ParserGeneratorValues.IdentifierName} || value.${ParserGeneratorValues.IdentifierName} !== val2.${ParserGeneratorValues.IdentifierName} || value !== val2`)
          .line('return false;');
