...
export class CommandTypeInfoImpl implements CommandTypeInfo, TypeChecker {
  ...
  constructor(dtdlVersion: number, id: string, childOf: string|undefined, definedIn: string|undefined, entityKind: EntityKind = EntityKind.COMMANDTYPE) {
...
.add('dtmi:dtdl:class:Entity');
    ...
    this._badTypeActionFormat[2] = `Choose one of the following values for property '{property}': asynchronous, synchronous.`;
    this._badTypeActionFormat[3] = `Choose one of the following values for property '{property}': asynchronous, synchronous.`;
  ...
  doesHaveType(typeId: string): boolean {
    return CommandTypeInfoImpl._versionlessTypes.has(new InDTMI(typeId).versionless) ||
this.supplementalTypes.some((st) => (st as SupplementalTypeInfoImpl).doesHaveType(typeId))
  ...
  static parseObject(model: Model, objectPropertyInfoList: ParsedObjectPropertyInfo[], elementPropertyConstraints: ElementPropertyConstraint[], valueConstraints: ValueConstraint[], aggregateContext: AggregateContext, parsingErrors: ParsingError[], object: {[index: string]: any}, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, dtmiSeg: string|undefined, keyProp: string|undefined, idRequired: boolean, typeRequired: boolean, allowIdReferenceSyntax: boolean, allowedVersions: Set<number>): any {
    // This is a method to parse the object read from DTDL into a type of CommandTypeInfo
    ...
    (elementInfo as CommandTypeInfoImpl).sourceObject = object;
    switch (childAggregateContext.dtdlVersion) {
    case 2: {
      (elementInfo as CommandTypeInfoImpl)?.parsePropertiesV2(model, objectPropertyInfoList, elementPropertyConstraints, childAggregateContext, parsingErrors, object, definedIn, allowIdReferenceSyntax);
      break;
    }

    case 3: {
      (elementInfo as CommandTypeInfoImpl)?.parsePropertiesV3(model, objectPropertyInfoList, elementPropertyConstraints, childAggregateContext, parsingErrors, object, definedIn, allowIdReferenceSyntax);
      break;
    }
    }
  ...
  static parseTypeArray(tokenArr: any[], elementId: string, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, aggregateContext: AggregateContext, parsingErrors: ParsingError[]): CommandTypeInfo|undefined {
    const materialKinds : EntityKind[] = [];
    const elementInfo : Reference<CommandTypeInfo> = referenceInit();
    ...
    for (const supplementalTypeId of supplementalTypeIds) {
      ...
      if (elementInfo.ref !== undefined && elementInfo.ref.entityKind !== undefined) {
        if (!(supplementalTypeInfo as SupplementalTypeInfoImpl)?.allowedCotypeKinds.includes(elementInfo.ref.entityKind)) {
          ...
          (elementInfo.ref as CommandTypeInfoImpl).addType(supplementalTypeId, supplementalTypeInfo);
        }
      }
    }
  ...
  static tryParseTypeStringV2(typestring: string, elementId: string, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, materialKinds: EntityKind[], supplementalTypeIds: string[], elementInfo: Reference<CommandTypeInfo>, undefinedTypes: string[], aggregateContext: AggregateContext, parsingErrors: ParsingError[]): boolean {
    switch (typestring) {
    case 'CommandType':
...
**/
  setPartitionInfo(partitionJsonText: string): void {
    throw new Error(`attempt to set partition info on non-partition type CommandTypeInfoInfo`);
  }
