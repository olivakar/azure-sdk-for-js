...
export class PartitionTypeCollectionGenerator implements TypeGenerator {
  ...
  generateCode(parserLibrary: TsLibrary): void {
    ...
    collectionClass.field({name: '_partitionTypeStrings', type: 'Set<string>', access: TsAccess.Private, isStatic: true});
    collectionClass.field({name: '_partitionTypeDescription', type: 'string', access: TsAccess.Private, isStatic: true});
    ...
    constructor.body.line(`Array.from(this._partitionTypeStrings.values()).map((s) => \`'\${s}'\`).join(' or ')`);
