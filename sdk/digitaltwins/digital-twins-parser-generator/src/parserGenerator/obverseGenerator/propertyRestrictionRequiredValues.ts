...
export class PropertyRestrictionRequiredValues implements PropertyRestriction {
  ...
  constructor(propertyName: string, values: string[], context: {[x: string]: string}) {
    ...
    this._conditionString = values.map((x) => `this.${propertyName}.${ParserGeneratorValues.IdentifierName} !== '${context[x]}'`).join(' && ');
    ...
    checkRestrictionsMethodBody
      .if(`this.${this._propertyName} !== undefined`)
        ...
        .line('parsingErrors.push(new ParsingError(')
            ...
            .line('{')
            .line(`cause: '{{primaryId:n}} property \\'${this._propertyName}\\' has value {value} , but the value must be \\'${this._requiredValues}\\'.',`)
            .line(`action: 'Change the value of property  \\'${this._propertyName}\\' to \\'${this._requiredValues}\\'.',`)
            .line(`primaryId: this.${ParserGeneratorValues.IdentifierName},`)
            .line(`property: '${this._propertyName}',`)
            .line(`value: AggregateContext.getTermOrUri(this.${this._propertyName}.${ParserGeneratorValues.IdentifierName}),`)
            .line(`}));`);
  }
}
