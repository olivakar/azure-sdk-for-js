...
export class ModelGenerator implements TypeGenerator {
  ...
  generateCode(parserLibrary: TsLibrary) {
    ...
    modelClass
      .import(`import {ParsingError, InDTMI} from '../parser';`)
  ...
  private _generateTrySetObjectPropertyMethod(modelClass: TsClass, referenceClassName: string) {
    ...
    trySetObjectPropertyMethod.body
        .line(`const obj = Object.keys(this.dict).includes(referencedElementId) ? this.dict[referencedElementId] : new ${referenceClassName}(0, referencedElementId, undefined, undefined);`)
