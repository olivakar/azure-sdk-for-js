...
export class StandardElementsGenerator implements TypeGenerator {
  ...
  generateCode(parserLibrary: TsLibrary): void {
    ...
    standardElementsClass.import(`import {ParsedObjectPropertyInfo, Model, ModelParserImpl, AggregateContext} from './internal';`);
    standardElementsClass.import(`import {InDTMI, ParsingError, ParsingException} from '../parser/internal';`);
    standardElementsClass.prefixCode.line('type EntityInfo = any;');
    standardElementsClass.docString.line('A collection of values of standard elements from the DTDL metamodel.');
  ...
  private _generateGetElementMethod(standardElementsClass: TsClass) {
    const method = standardElementsClass.method({name: 'getElement', returnType: this._baseClassName, isStatic: true});
  ...
  private _generateGetDigestElementsMethod(standardElementsClass: TsClass) {
    const method = standardElementsClass.method({name: 'getDigestElements', returnType: 'any', isStatic: true});
