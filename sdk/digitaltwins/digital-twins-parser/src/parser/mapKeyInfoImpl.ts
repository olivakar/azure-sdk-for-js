...
export class MapKeyInfoImpl implements MapKeyInfo, TypeChecker {
  ...
  doesHaveType(typeId: string): boolean {
    return (
      MapKeyInfoImpl._versionlessTypes.has(new InDTMI(typeId).versionless) ||
  ...
  static parseObject(
    ...
    switch (childAggregateContext.dtdlVersion) {
      case 2: {
        (elementInfo as MapKeyInfoImpl)?.parsePropertiesV2(
          model,
          objectPropertyInfoList,
          elementPropertyConstraints,
          childAggregateContext,
      ...
      case 3: {
        (elementInfo as MapKeyInfoImpl)?.parsePropertiesV3(
          model,
          objectPropertyInfoList,
          elementPropertyConstraints,
          childAggregateContext,
  ...
  static tryParseTypeStringV2(
    ...
    elementInfo: Reference<MapKeyInfo>,
    ...
    for (const propKey in object) {
      ...
      switch (propKey) {
        ...
        case "dtmi:dtdl:property:schema;2":
          ...
          valueCount = StringInfoImpl.parseToken(
            model,
            objectPropertyInfoList,
            elementPropertyConstraints,
  ...
  checkRestrictionsV2(parsingErrors: ParsingError[]) {
    if (this._schemaInstanceProperties !== undefined) {
      for (const instanceProp of this._schemaInstanceProperties) {
        if (
          !(this.schema as StringInfoImpl)?.validateInstanceElement(
