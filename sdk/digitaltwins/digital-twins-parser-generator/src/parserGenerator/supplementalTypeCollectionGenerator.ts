...
export class SupplementalTypeCollectionGenerator implements TypeGenerator {
  ...
  generateCode(parserLibrary: TsLibrary) {
    const collectionInterface = parserLibrary.interface({name: 'SupplementalTypeCollection', exports: true});
    collectionInterface.field({name: 'supplementalTypes', type: 'Map<string, SupplementalTypeInfo>'});
    collectionInterface.import(`import {SupplementalTypeInfo} from './internal';`);

    const collectionClass = parserLibrary.class({name: 'SupplementalTypeCollectionImpl', exports: true});
    collectionClass.docString.line('A collection of DTDL types that are not materialized as TS Classes');
    collectionClass.import(`import {SupplementalTypeInfo, SupplementalTypeInfoImpl, SupplementalTypeCollection} from './internal';`);
