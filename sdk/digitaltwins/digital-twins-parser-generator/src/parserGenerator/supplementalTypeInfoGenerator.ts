...
export class SupplementalTypeInfoGenerator implements TypeGenerator {
  ...
  constructor(baseName: string) {
    this._baseEnumName = NameFormatter.formatNameAsEnum(baseName);
  }
  ...
  generateCode(parserLibrary: TsLibrary) {
    const infoInterface : TsInterface = parserLibrary.interface({name: 'SupplementalTypeInfo', exports: true});
    infoInterface.import(`import {SupplementalPropertyInfo} from './internal'`);
    infoInterface.docString.line('Interface that provides information about a type is not materialized as a material class.');
    ...
    const infoClass: TsClass = parserLibrary.class({name: 'SupplementalTypeInfoImpl', exports: true, inheritance: [{name: 'SupplementalTypeInfo', type: TsDeclarationType.Interface}]});
    infoClass.import(`import {PropertyConstraint} from '../parser';`);
    infoClass.import(`import {SupplementalPropertyInfoImpl, SupplementalTypeInfo} from './internal'`);
    infoClass.import(`import {ElementPropertyConstraint} from '../parser'`);
    infoClass.import(`import {InDTMI} from '../parser'`);
    infoClass.import(`import {ParsingError} from '../parser'`);
    ...
    infoClass.docString.line('Class that provides information about a type is not materialized as a TS class.');

    infoClass.field({name: 'allowedCotypeKinds', type: `${this._baseEnumName}[]`, readonly: true});
    ...
    const addCotypeMethod = infoClass.method({
      name: 'addCotype',
      returnType: 'void',
    }).parameter({name: 'cotypeKind', type: `${this._baseEnumName}`});
    addCotypeMethod.body
      .line(`this.allowedCotypeKinds.push(cotypeKind);`);
