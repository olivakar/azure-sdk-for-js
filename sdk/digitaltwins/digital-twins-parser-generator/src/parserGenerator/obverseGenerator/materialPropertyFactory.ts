...
export class MaterialPropertyFactory {
  ...
  public static create(dtdlVersions:number[], propertyName: string, propertyToken: MaterialPropertyDigest, dtdlContexts : {[contextId: string]: Context}, baseClassname: string) : MaterialProperty|undefined {
    const propertyNameUris : {[dtdlVersion: number]: string} = {};
    for (const dtdlVersion of dtdlVersions) {
      propertyNameUris[dtdlVersion] = dtdlContexts[ParserGeneratorValues.getDtdlContextIdString(dtdlVersion)][propertyName];
    }
    // TODO Property Restrictions.
    const propertyRestrictions = this.getPropertyRestrictions(dtdlContexts, propertyName, propertyToken);
    if (propertyToken._.literal) {
      if (propertyToken._.datatype !== undefined) {
        if (propertyToken._.datatype === 'langString') {
          return new LangStringLiteralProperty(propertyName, propertyNameUris, propertyToken, propertyRestrictions);
        }
        ...
        if (propertyToken._.plural) {
          ...
          return new PluralTypedLiteralProperty({propertyName: propertyName, propertyNameUris: propertyNameUris, propertyDigest: propertyToken, propertyRestrictions: propertyRestrictions, datatype: propertyToken._.datatype, literalType: literalType});
        } else {
          // TODO: This will eventually get implemented, and idk why will be obversePropertyName....
          return new SingularTypedLiteralProperty({propertyName: propertyName, propertyNameUris: propertyNameUris, propertyDigest: propertyToken, propertyRestrictions: propertyRestrictions, datatype: propertyToken._.datatype, literalType: literalType});
        }
      } else {
        if (propertyToken._.plural) {
          return new PluralUntypedLiteralProperty(propertyName, propertyNameUris, propertyToken, propertyRestrictions);
        } else {
          return new SingularUntypedLiteralProperty(propertyName, propertyNameUris, propertyToken, propertyRestrictions);
        }
      }
    } else if (propertyToken._.class !== undefined) {
      if (propertyToken._.dictionaryKey !== undefined) {
        return new DictionaryObjectProperty(propertyName, propertyNameUris, propertyToken, propertyRestrictions);
      } else if (propertyToken._.plural) {
        return new PluralObjectProperty(propertyName, propertyNameUris, propertyToken, propertyRestrictions);
      } else {
        return new SingularObjectProperty(propertyName, propertyNameUris, propertyToken, propertyRestrictions);
      }
    } else {
      if (propertyToken._.plural) {
        return new PluralIdentifierProperty(propertyName, propertyNameUris, propertyToken, propertyRestrictions, baseClassname);
      } else {
        return new SingularIdentifierProperty(propertyName, propertyNameUris, propertyToken, propertyRestrictions, baseClassname);
      }
    }
  }

  private static getPropertyRestrictions(dtdlContexts : {[contextId: string]: Context}, propertyName:string, propertyDigest:MaterialPropertyDigest) : {[x: number]: PropertyRestriction[]} {
    const propertyRestrictions : {[version: number]: PropertyRestriction[]} = {};

    Object.entries(propertyDigest).forEach(
      ([version, versionedValue]) => {
        ...
        if (!Object.prototype.hasOwnProperty.call(propertyRestrictions, version) && propertyRestrictions[version as unknown as number] === undefined) {
          propertyRestrictions[version as unknown as number] = verisonedRestrictions;
        }
        if (versionedValue.values !== undefined) {
          verisonedRestrictions.push(new PropertyRestrictionRequiredValues(propertyName, versionedValue.values, dtdlContexts[ParserGeneratorValues.getDtdlContextIdString(version as unknown as number)]));
        }
        if (versionedValue.uniqueProperties !== undefined) {
          ...
          for (const uniquePropName of versionedValue.uniqueProperties) {
            if (uniquePropName !== propertyDigest._.dictionaryKey) {
              verisonedRestrictions.push(new PropertyRestrictionUniqueProperties(propertyName, uniquePropName));
            }
          }
        }
      },
