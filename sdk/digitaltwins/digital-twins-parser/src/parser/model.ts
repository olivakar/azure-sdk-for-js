...
import {ParsedObjectPropertyInfo, ReferenceInfoImpl, ModelDict, SupplementalTypeInfo, EntityKinds, EntityInfoImpl} from './internal';
/**
 * A DTDL model.
**/
export class Model {
  ...
  trySetObjectProperty(elementId: string, propertyName: string, referencedElementId: string, key?: string): boolean {
    const obj = Object.keys(this.dict).includes(referencedElementId) ? this.dict[referencedElementId] : new ReferenceInfoImpl(0, referencedElementId, undefined, undefined, 'reference');
  ...
  setObjectProperties(objectPropertyInfoList: ParsedObjectPropertyInfo[], parsingErrors: ParsingError[]) {
    for (const objectPropertyInfo of objectPropertyInfoList) {
      if (!((this.dict[objectPropertyInfo.referencedElementId] as EntityInfoImpl)?.isPartition)) {
        ...
        const sourcePartition: string | undefined = (elementIdValue?.isPartition || elementIdValue?.definedIn === undefined) ? objectPropertyInfo.elementId : elementIdValue?.definedIn;
