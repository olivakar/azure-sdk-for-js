...
export class MapKeyInfoImpl implements MapKeyInfo, TypeChecker {
  ...
  private _schemaAllowedVersionsV3: Set<number> = new Set<number>().add(3);
  ...
  constructor(dtdlVersion: number, id: string, childOf: string|undefined, definedIn: string|undefined, entityKind: EntityKind = EntityKind.MAPKEY) {
...
.add('dtmi:dtdl:class:NamedEntity');
    ...
    this._badTypeActionFormat[2] = `Provide a value for property '{property}' with @type MapKey.`;
    this._badTypeActionFormat[3] = `Provide a value for property '{property}' with @type MapKey.`;
    this._badTypeCauseFormat[2] = `{primaryId:p} property '{property}' has value{secondaryId:e} that does not have @type of MapKey.`;
    this._badTypeCauseFormat[3] = `{primaryId:p} property '{property}' has value{secondaryId:e} that does not have @type of MapKey.`;
  }
  ...
  doesHaveType(typeId: string): boolean {
    return MapKeyInfoImpl._versionlessTypes.has(new InDTMI(typeId).versionless) ||
this.supplementalTypes.some((st) => (st as SupplementalTypeInfoImpl).doesHaveType(typeId))
  ...
  static parseObject(model: Model, objectPropertyInfoList: ParsedObjectPropertyInfo[], elementPropertyConstraints: ElementPropertyConstraint[], valueConstraints: ValueConstraint[], aggregateContext: AggregateContext, parsingErrors: ParsingError[], object: {[index: string]: any}, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, dtmiSeg: string|undefined, keyProp: string|undefined, idRequired: boolean, typeRequired: boolean, allowIdReferenceSyntax: boolean, allowedVersions: Set<number>): any {
    // This is a method to parse the object read from DTDL into a type of MapKeyInfo
    ...
    (elementInfo as MapKeyInfoImpl).sourceObject = object;
    switch (childAggregateContext.dtdlVersion) {
    case 2: {
      (elementInfo as MapKeyInfoImpl)?.parsePropertiesV2(model, objectPropertyInfoList, elementPropertyConstraints, childAggregateContext, parsingErrors, object, definedIn, allowIdReferenceSyntax);
      break;
    }

    case 3: {
      (elementInfo as MapKeyInfoImpl)?.parsePropertiesV3(model, objectPropertyInfoList, elementPropertyConstraints, childAggregateContext, parsingErrors, object, definedIn, allowIdReferenceSyntax);
      break;
    }
    }
  ...
  static parseTypeArray(tokenArr: any[], elementId: string, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, aggregateContext: AggregateContext, parsingErrors: ParsingError[]): MapKeyInfo|undefined {
    const materialKinds : EntityKind[] = [];
    const elementInfo : Reference<MapKeyInfo> = referenceInit();
    ...
    for (const supplementalTypeId of supplementalTypeIds) {
      ...
      if (elementInfo.ref !== undefined && elementInfo.ref.entityKind !== undefined) {
        if (!(supplementalTypeInfo as SupplementalTypeInfoImpl)?.allowedCotypeKinds.includes(elementInfo.ref.entityKind)) {
          ...
          (elementInfo.ref as MapKeyInfoImpl).addType(supplementalTypeId, supplementalTypeInfo);
        }
      }
    }
  ...
  static tryParseTypeStringV2(typestring: string, elementId: string, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, materialKinds: EntityKind[], supplementalTypeIds: string[], elementInfo: Reference<MapKeyInfo>, undefinedTypes: string[], aggregateContext: AggregateContext, parsingErrors: ParsingError[]): boolean {
    switch (typestring) {
    case 'MapKey':
  ...
  parsePropertiesV2(model: Model, objectPropertyInfoList: ParsedObjectPropertyInfo[], elementPropertyConstraints: ElementPropertyConstraint[], aggregateContext: AggregateContext, parsingErrors: ParsingError[], object: {[index: string]: any}, definedIn: string|undefined, allowIdReferenceSyntax: boolean): void {
    ...
    for (const propKey in object) {
      ...
      case 'dtmi:dtdl:property:schema;2':
        ...
        valueCount = StringInfoImpl.parseToken(model, objectPropertyInfoList, elementPropertyConstraints, this._schemaValueConstraints, aggregateContext, parsingErrors, propValue, this.id, definedIn ?? this.id, 'schema', undefined, undefined, true, true, allowIdReferenceSyntax, this._schemaAllowedVersionsV2);
        if (valueCount < 1) {
          parsingErrors.push(new ParsingError(
  ...
  parsePropertiesV3(model: Model, objectPropertyInfoList: ParsedObjectPropertyInfo[], elementPropertyConstraints: ElementPropertyConstraint[], aggregateContext: AggregateContext, parsingErrors: ParsingError[], object: {[index: string]: any}, definedIn: string|undefined, allowIdReferenceSyntax: boolean): void {
    ...
    for (const propKey in object) {
      ...
      case 'dtmi:dtdl:property:schema;3':
        schemaPropertyMissing = false;
        valueCount = StringInfoImpl.parseToken(model, objectPropertyInfoList, elementPropertyConstraints, this._schemaValueConstraints, aggregateContext, parsingErrors, propValue, this.id, definedIn ?? this.id, 'schema', undefined, undefined, true, true, allowIdReferenceSyntax, this._schemaAllowedVersionsV3);
        if (valueCount < 1) {
          parsingErrors.push(new ParsingError(
            'dtmi:dtdl:parsingError:propertyCountBelowMin',
            {
...
**/
  setPartitionInfo(partitionJsonText: string): void {
    throw new Error(`attempt to set partition info on non-partition type MapKeyInfoInfo`);
  }
  ...
  checkRestrictionsV2(parsingErrors: ParsingError[]) {
    if (this._schemaInstanceProperties !== undefined) {
      for (const instanceProp of this._schemaInstanceProperties) {
        if (!(this.schema as StringInfoImpl)?.validateInstanceElement(this.supplementalProperties[instanceProp])) {
          parsingErrors.push(new ParsingError(
  ...
  trySetObjectProperty(propertyName: string, value: any, key: string|undefined): boolean {
    ...
    case 'dtmi:dtdl:property:schema;2':
    case 'dtmi:dtdl:property:schema;3':
      this.schema = value as StringInfoImpl;
      return true;
    default:
      break;
    }
...
**/
  checkDepthOfElementSchemaOrSchema(depth: number, depthLimit: number, tooDeepElementId: Reference<InDTMI>, parsingErrors: ParsingError[]): boolean {
    ...
    if (this.schema !== undefined) {
      if (!(this.schema as StringInfoImpl).checkDepthOfElementSchemaOrSchema(depth + 1, depthLimit, tooDeepElementId, parsingErrors)) {
  ...
  tryGetDescendantSchemaOrContentsComponentNarrow(elementId: Reference<InDTMI>): boolean {
    ...
    if (this.schema !== undefined) {
      if ((this.schema as StringInfoImpl).entityKind === EntityKind.COMPONENT) {
        elementId.ref = new InDTMI((this.schema as StringInfoImpl).id);
        this._idOfDescendantSchemaOrContentsComponentNarrow = elementId.ref;
        return true;
      }

      if ((this.schema as StringInfoImpl).tryGetDescendantSchemaOrContentsComponentNarrow(elementId)) {
        this._idOfDescendantSchemaOrContentsComponentNarrow = elementId.ref;
        return true;
      }
    }
  ...
  checkDescendantEnumValueDataType(ancestorId: InDTMI, datatype: string, parsingErrors: ParsingError[]): void {
    ...
    if (this.schema !== undefined) {
      (this.schema as StringInfoImpl).checkDescendantEnumValueDataType(ancestorId, datatype, parsingErrors);
    }
  }
  ...
  tryGetDescendantSchemaArray(elementId: Reference<InDTMI>): boolean {
    ...
    if (this.schema !== undefined) {
      if ((this.schema as StringInfoImpl).entityKind === EntityKind.ARRAY) {
        elementId.ref = new InDTMI((this.schema as StringInfoImpl).id);
        this._idOfDescendantSchemaArray = elementId.ref;
        return true;
      }

      if ((this.schema as StringInfoImpl).tryGetDescendantSchemaArray(elementId)) {
        this._idOfDescendantSchemaArray = elementId.ref;
        return true;
      }
    }
  ...
  getCountOfContentsOrFieldsOrEnumValuesOrRequestOrResponseOrPropertiesOrSchemaOrElementSchemaOrMapValueNarrow(parsingErrors: ParsingError[]): number {
    ...
    if (this.schema !== undefined) {
      this._countOfContentsOrFieldsOrEnumValuesOrRequestOrResponseOrPropertiesOrSchemaOrElementSchemaOrMapValueNarrowValue += (this.schema as StringInfoImpl).getCountOfContentsOrFieldsOrEnumValuesOrRequestOrResponseOrPropertiesOrSchemaOrElementSchemaOrMapValueNarrow(parsingErrors) + 1;
    }
