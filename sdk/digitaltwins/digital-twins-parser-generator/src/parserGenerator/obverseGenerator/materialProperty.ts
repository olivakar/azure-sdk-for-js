...
export abstract class MaterialProperty {
  ...
  constructor(propertyName: string, propertyNameUris: {[dtdlVersion: number]: string}, propertyToken: MaterialPropertyDigest, propertyRestrictions: {[dtdlVersion: number]: PropertyRestriction[]}) {
    ...
    // this._datatype = propertyToken._.datatype;
    this._isLiteral = propertyToken._.literal ?? false;
    // this._class = propertyToken._.class;
  ...
  // protected get datatype() : string|undefined {
  ...
  public addMembers(dtdlVersions:number[], obverseClass:TsClass, obverseInterface:TsInterface, classIsAugmentable:boolean) : void {
    ...
    //   obverseClass.field({name: fieldName +'?', type: this.propertyType !== undefined? this.propertyType : 'any', access: TsAccess.Protected});
    //   obverseClass.getter({name: this.propertyName, returnType: `${this.propertyType}|undefined`, access: TsAccess.Public})
    //       .body
    //       .line(`return this.${fieldName};`);
    // }
    // if (!this.inherited) {
    //   // the field will have a different class than the interface.
    //   obverseClass.field({name: this.propertyName +'?', type: this.propertyImplType !== undefined? this.propertyImplType : 'any', access: TsAccess.Public});
  ...
  public addImports(obverseInterface:TsInterface) : void {
    // obverseInterface.import(`import {${this._typeName}} from './internal';`);
  }
