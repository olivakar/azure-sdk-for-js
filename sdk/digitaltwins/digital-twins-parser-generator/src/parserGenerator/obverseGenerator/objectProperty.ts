...
export abstract class ObjectProperty extends MaterialProperty {
  ...
  constructor(propertyName: string, propertyNameUris :{[dtdlVersion: number]: string}, propertyToken: MaterialPropertyDigest, propertyRestrictions: {[dtdlVersion: number]: PropertyRestriction[]}) {
    ...
    this._interfaceName = propertyToken._.class !== undefined ? NameFormatter.formatNameAsInterface(propertyToken._.class) : undefined;
    this._implementationName = propertyToken._.class !== undefined ? NameFormatter.formatNameAsImplementation(propertyToken._.class) : undefined;
    this._versionedClassName = {};
    for (const keyDtdlVersion in propertyToken) {
      if (keyDtdlVersion === '_') { // skip underscore
        continue;
      }
      const versionedClassName = propertyToken[keyDtdlVersion].class;
      if (versionedClassName !== undefined) {
        this._versionedClassName[keyDtdlVersion] = NameFormatter.formatNameAsImplementation(versionedClassName);
      }
    }
  ...
  public hasCountRestriction(dtdlVersion:number) : boolean {
    return Object.prototype.hasOwnProperty.call(this.propertyDigest, dtdlVersion) && (this.propertyDigest[dtdlVersion].minCount !== undefined || this.propertyDigest[dtdlVersion].maxCount !== undefined);
  }
  ...
  public addMembers(dtdlVersions: number[], obverseClass: TsClass, obverseInterface:TsInterface, classIsAugmentable:boolean) : void {
    ...
    //   obverseClass.field({name: `_${this.valueConstraintsField}`, access: TsAccess.Protected, type: 'ValueConstraint[]', value: '[]'});
    //   obverseClass.field({name: `_${this.instancePropertiesField}`, access: TsAccess.Protected, type: 'string[]', value: '[]'});
    // }
    ...
    //     }
    //     obverseClass.field({name: `_${this.allowedVersionsField}V${dtdlVersion}`, access: TsAccess.Protected, type: 'Set<number>', value: value});
    //   }
    // }
    if (Object.values(this.propertyDigest).some((versionedDigest) => versionedDigest.allowed) && classIsAugmentable) {
      obverseClass.field({name: `_${this.valueConstraintsField}`, access: TsAccess.Private, type: 'ValueConstraint[]', value: '[]'});
      obverseClass.field({name: `_${this.instancePropertiesField}`, access: TsAccess.Private, type: 'string[]', value: '[]'});
    }
    for (const dtdlVersion of dtdlVersions) {
      const propertyVersionDigest = this.propertyDigest[dtdlVersion];
      // TODO Ask JD to show generated code
      ...
      if (dictVersions !== undefined) {
        ...
        obverseClass.field({name: `_${this.allowedVersionsField}V${dtdlVersion}`, access: TsAccess.Private, type: 'Set<number>', value: value});
      }
    }
  }
  ...
  public addCaseForValueConstraintSwitch(switchOnProperty:TsScope, constraintVariable:string) : void {
    if (Object.values(this.propertyDigest).some((versionedDigest) => versionedDigest.allowed)) {
      switchOnProperty.line(`case '${this.propertyName}':`);
      switchOnProperty
