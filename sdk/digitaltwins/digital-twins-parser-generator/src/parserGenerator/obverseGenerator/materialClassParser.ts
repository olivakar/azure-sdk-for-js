...
import {TypeVersionToConcreteSubclasses, TypeVersionToExtensibleMaterialClasses, TypeVersionToExtensibleMaterialSubTypes} from '../materialClassGenerator';
...
export class MaterialClassParser {
  ...
  public static addMembers(dtdlVersions: number[], obverseClass:TsClass, obverseInterface:TsInterface, rawTypeName:string, typeName: string,
    kindEnum:string, kindProperty:string, classIsBase:boolean, classIsAbstract:boolean,
    classIsPartition:boolean, dtdlVersionToConcreteSubclasses:TypeVersionToConcreteSubclasses,
    dtdlVersionToExtensibleClasses: TypeVersionToExtensibleMaterialClasses, dtdlVersionExtensibleMaterialSubtypes:TypeVersionToExtensibleMaterialSubTypes,
    ...
    this._generateParseObjectMethod(dtdlVersions, obverseClass, rawTypeName, typeName, classIsBase, classIsAbstract);
    this._generateParseTypeArrayMethod(dtdlVersions, obverseClass, typeName, kindEnum, kindProperty, classIsBase);
    ...
    for (const dtdlVersion of dtdlVersions) {
      this._generateTryParseTypeStringMethod(
        dtdlVersion, obverseClass, typeName, kindEnum,
  ...
  private static _generateDoesHaveTypeMethod(obverseClass: TsClass, classIsAbstract: boolean) {
    const method = obverseClass.method({name: 'doesHaveType', returnType: 'boolean', abstract: false, isStatic: false})
                                .parameter({name: 'typeId', type: 'string'});

    const returnLine = method.body.line(`return ${obverseClass.name}._versionlessTypes.has(new InDTMI(typeId).versionless)`);
    MaterialClassAugmentor.addTypeCheckLine(returnLine, !classIsAbstract);
    returnLine.line(';');
  }

  private static _generateUndefinedTypeAndPropertiesProperties(obverseClass: TsClass, obverseInterface:TsInterface, classIsBase: boolean) {
    ...
    //   const fName = 'undefinedTypes';
    //   const fType = 'string[]';
    //   obverseClass.field({name: `_${fName}`, access: TsAccess.Protected, type: `${fType}`});
    //   obverseClass
    //     .getter({name: `${fName}`, access: TsAccess.Public, returnType: `${fType}`})
    //     .body
    //       .line(`return this._${fName}`);

    //   obverseClass
    //     .setter({name: `${fName}`, access: TsAccess.Public})
    //     .parameter({name: 'input', type: `${fType}`})
    //     .body
    //       .line(`this._${fName} = input;`);

    //   const fNameProps = 'undefinedProperties';
    //   const fTypeProps = '{[name: string]: any}';
    //   obverseClass.field({name: `_${fNameProps}`, access: TsAccess.Protected, type: `${fTypeProps}`});
    //   obverseClass
    //         .getter({name: `${fNameProps}`, access: TsAccess.Public, returnType: `${fTypeProps}`})
    //         .body
    //           .line(`return this._${fNameProps}`);

    //   obverseClass
    //         .setter({name: `${fNameProps}`, access: TsAccess.Public})
    //         .parameter({name: 'input', type: `${fTypeProps}`})
    //         .body
    //           .line(`this._${fNameProps} = input;`);
    // }
    // TODO Not sure if any other properties are needed.
  }

  private static _generateParseObjectMethod(dtdlVersions:number[], obverseClass: TsClass, rawTypeName:string, typeName:string, classIsBase:boolean, classIsAbstract:boolean) : void {
    const typeNameImpl = typeName + 'Impl';
    obverseClass.import(`import {IdValidator} from './internal';`);
    obverseClass.import(`import {ParsingError} from '../parser/parsingError';`);
    ...
    obverseClass.import(`import {ElementPropertyConstraint, ValueParser, ValueConstraint} from '../parser';`);

    const parseObjMethod = obverseClass.method({name: 'parseObject', returnType: 'any', abstract: false, isStatic: true})
    ...
    parseObjMethod.body
      ...
      .line(`const childAggregateContext = aggregateContext.getChildContext(object, parsingErrors);`);
    parseObjMethod.body
      .if(`Object.keys(object).length === 1 && Object.prototype.hasOwnProperty.call(object, '@id') && typeof object['@id'] === 'string'`)
        .if(`allowIdReferenceSyntax && parentId!== undefined`)
          .line(`this.parseIdString(objectPropertyInfoList, elementPropertyConstraints, valueConstraints, childAggregateContext, parsingErrors, object['@id'], parentId, propName, keyProp, allowedVersions);`)
          .line('return;')
        .else()
          .line('parsingErrors.push(new ParsingError(')
          .line(`'dtmi:dtdl:parsingError:idReference',`)
          .line('{')
          .line(`cause: \`{primaryId:p} property '{property}' has an inline definition containing nothing but an '@id' property.\`,`)
          .line(`action: \`Replace the inline definition with a string value of '{secondaryId}', or provide a complete inline definition for property '{property}'.\`,`)
    ...
    parseObjMethod.body
      .if(`allowedVersions !== undefined && allowedVersions.size !== 0 && !allowedVersions.has(childAggregateContext.dtdlVersion)`)
        ...
        .line('{')
        .line('cause: `{primaryId:p} property \'{property}\' has a value that specifies DTDL context version ${childAggregateContext.dtdlVersion}, which is not allowed for this property.`,')
        .line('action: `Change the DTDL context version of property \'{property}\' to one of the following: ${Array.from(allowedVersions.values()).join(\' ,\')}.`,')
        .line(`primaryId: parentId,`)
        .line(`property: propName,`)
        .line(`}));`);
    parseObjMethod.body
      ...
      .line('const elementId = IdValidator.parseIdProperty(object, parentId !== undefined? parentId:\'\', propName, dtmiSeg, idRequired, parsingErrors, childAggregateContext.dtdlVersion);')
    ...
    autogennedIfScope
        .if(`!elementDtmi?.isReserved`)
          ...
          .line('action: `Remove all but one JSON object containing \'@id\' property with value {primaryId}, or change the \'@id\' values so there are no duplicates.`,')
        ...
        .elseIf(`dtmiSeg !== undefined`)
          ...
          .line('{')
          .line('cause: `{primaryId:p} property ${propName} contains more than one element whose property \'{dtmiSeg}\' has value ${dtmiSeg}`,')
          .line('action: `Change the value of property ${dtmiSeg} to a string value that is unique across all values of ${propName}.`,')
          .line(`primaryId: parentId,`)
          .line(`property: propName,`)
          .line(`value: dtmiSeg`)
          .line(`}));`);
    ...
    parseObjMethod.body
      .line(`const elementInfo = this.parseTypeArray(typeTokenArr, elementId, parentId, definedIn, propName, childAggregateContext, parsingErrors);`)
    ...
    for (const dtdlVersion of dtdlVersions) {
      ...
      switchCase
      .line(`(elementInfo as ${typeNameImpl})?.parsePropertiesV${dtdlVersion}(model, objectPropertyInfoList, elementPropertyConstraints, childAggregateContext, parsingErrors, object, definedIn, allowIdReferenceSyntax);`)
      .line(`break;`);
    }
    parseObjMethod.body
      .line(`model.dict[elementId] = elementInfo;`)
      .if('parentId !== undefined')
        .line('const objectPropertyInfo: ParsedObjectPropertyInfo = {elementId: parentId, propertyName: propName || \'\', referencedElementId: elementId, keyProperty: keyProp, expectedKinds: [], allowedVersions: new Set<number>(), badTypeCauseFormat: undefined, badTypeActionFormat: undefined};')
        ...
        .if('valueConstraints !== undefined && elementPropertyConstraints !== undefined')
          .for('const vc of valueConstraints')
              .line('const elementPropertyConstraint = {parentId: parentId, propertyName: propName, elementId: elementId, valueConstraint: vc,};')
              .line('elementPropertyConstraints.push(elementPropertyConstraint);');
  }

  private static _generateParseTypeArrayMethod(dtdlVersions:number[], tsClass:TsClass, typeName:string, kindEnum:string, kindProperty:string, classIsBase:boolean) : void {
    const typeNameImpl = typeName + 'Impl';
    const parseTypeArrayMethod = tsClass.method({name: 'parseTypeArray', returnType: `${typeName}|undefined`, abstract: false, isStatic: true})
    ...
    const forloop = parseTypeArrayMethod.body
    ...
    if (dtdlVersions.length > 0) {
      ...
      for (const dtdlVersion of dtdlVersions) {
        ...
        switchCase.if(`!this.tryParseTypeStringV${dtdlVersion}(element.toString(), elementId, parentId, definedIn, propName, materialKinds, supplementalTypeIds, elementInfo, undefinedTypes, aggregateContext, parsingErrors)`)
    ...
    parseTypeArrayMethod.body
        .if(`materialKinds.length > 1`)
          ...
          .line('{')
          .line('cause: `{primaryId:p} has @type that specifies multiple material types: ${materialKinds.join(\' ,\')}`,')
    ...
    parseTypeArrayMethod.body
      .line(`elementInfo.ref.undefinedTypes = undefinedTypes;`);

    tsClass.import(`import {ModelParserImpl} from './internal';`);
    parseTypeArrayMethod.body
      .for(`const supplementalTypeId of supplementalTypeIds`)
        .line(`const supplementalTypeInfo = ModelParserImpl.retrieveSupplementalTypeCollection().supplementalTypes.get(supplementalTypeId);`)
          .if(`elementInfo.ref !== undefined && elementInfo.ref.entityKind !== undefined`)
            .if(`!(supplementalTypeInfo as SupplementalTypeInfoImpl)?.allowedCotypeKinds.includes(elementInfo.ref.entityKind)`)
              ...
              .line('{')
              .line(`cause: \`{primaryId:p} has @type {value} that can only be applied to elements of @type \${(supplementalTypeInfo as SupplementalTypeInfoImpl)?.allowedCotypeKinds.join(' or ')} + '.'\`,`)
              .line(`action: \`Remove @type '{value}' from element.\`,`)
              .line(`primaryId: elementId,`)
              .line(`value: AggregateContext.getTermOrUri(supplementalTypeId)`)
              .line(`}));`)
            .elseIf(`!(supplementalTypeInfo as SupplementalTypeInfoImpl)?.allowedCotypeVersions.includes(elementInfo.ref.dtdlVersion)`)
              .line('parsingErrors.push(new ParsingError(')
              .line(`'dtmi:dtdl:parsingError:invalidCotypeVersion',`)
              .line('{')
              .line(`cause: \`{primaryId:p} has @type {value} that can only be applied to elements defined in DTDL version \${(supplementalTypeInfo as SupplementalTypeInfoImpl)?.allowedCotypeVersions.join(' or ')} + '.'\`,`)
              .line(`action: \`Remove @type '{value}' from element.\`,`)
              .line(`primaryId: elementId,`)
              .line(`value: AggregateContext.getTermOrUri(supplementalTypeId)`)
              .line(`}));`)
            .else()
              .line(`(elementInfo.ref as ${typeNameImpl}).addType(supplementalTypeId, supplementalTypeInfo);`);

    parseTypeArrayMethod.body
    .line(`return elementInfo.ref;`)
    .line('// this ends the method.');
  }

  private static _generateDoesPropertyDictContainKeyMethod(obverseClass: TsClass, classIsBase: boolean, properties: MaterialProperty[]): void {
    const method = obverseClass.method({name: 'doesPropertyDictContainKey', returnType: 'boolean'});
    method
      .parameter({name: 'propertyName', type: 'string'})
      .parameter({name: 'key', type: 'string | undefined'});
    // if (!classIsBase) {
    //   method.body
    //           .if('super.doesPropertyDictContainKey(propertyName, key)')
    //           .line('return true');
    // }
  ...
  private static _generateTryParseTypeStringMethod(dtdlVersion:number, tsClass: TsClass, typeName:string, kindEnum:string, concreteSubclasses: ConcreteSubclass[], extensibleMaterialClasses: ExtensibleMaterialClass[], extensibleMaterialSubtypes: string[]) : void {
    const methodName = 'tryParseTypeStringV' + dtdlVersion;
    const tryParseTypeStringMethod = tsClass.method({name: methodName, returnType: 'boolean', abstract: false, isStatic: true})
    ...
    concreteSubclasses.forEach((subclass) => {
      ...
      subclass.addCaseToParseTypeStringSwitch(tryParseTypeStringMethod.body, 'elementInfo.ref', 'elementId', 'parentId', 'definedIn');
    });
    ...
    tryParseTypeStringMethod.body.line('const supplementalTypeId = aggregateContext.createDtmi(typestring);');
    const supplementalTypeIdDefinedIfScope = tryParseTypeStringMethod.body
        .if('supplementalTypeId === undefined');
    supplementalTypeIdDefinedIfScope
      .if('undefinedTypes === undefined')
        .line('undefinedTypes = [];');
    supplementalTypeIdDefinedIfScope
      .line('undefinedTypes.push(typestring);')
      .line('return true');

    tryParseTypeStringMethod.body
      .line('const mapOfInDTMIToSupplementalTypeInfo = ModelParserImpl.retrieveSupplementalTypeCollection().supplementalTypes;')
    ...
    const supplementalTypeIdDefinedIfScopeOuter = tryParseTypeStringMethod.body
      .if('supplementalTypeId !== undefined')
        .line('const supplementalTypeInfo = mapOfInDTMIToSupplementalTypeInfo.get(supplementalTypeId.value);');
    supplementalTypeIdDefinedIfScopeOuter
      .if('supplementalTypeInfo?.isAbstract')
        ...
        .line('{')
        .line(`cause: \`{primaryId:p} has @type that specifies supplemental type {value} that is abstract.\`,`)
    ...
    if (extensibleMaterialClasses !== undefined && extensibleMaterialClasses.length > 0) {
      const switchOnExtensionKind = supplementalTypeIdDefinedIfScopeOuter.scope('switch ((supplementalTypeInfo as SupplementalTypeInfoImpl)?.extensionKind)');
      tsClass.import(`import {ExtensionKind} from './internal';`);
      extensibleMaterialClasses.forEach((extensibleMaterialClass) => {
        extensibleMaterialClass.addCaseToParseTypeStringSwitch(tsClass, switchOnExtensionKind, extensibleMaterialSubtypes, 'parentId', 'definedIn');
      });
    }
    supplementalTypeIdDefinedIfScopeOuter
  ...
  private static _generateParsePropertiesMethod(dtdlVersion:number, obverseClass: TsClass, typeName:string, classIsBase:boolean, classIsAbstract:boolean, classIsPartition:boolean, materialProperties:MaterialProperty[]) : void {
    ...
    const parsePropertiesMethod = obverseClass.method({name: methodName, returnType: 'void', abstract: false, isStatic: false})
      .parameter({name: 'model', type: 'Model'})
      .parameter({name: 'objectPropertyInfoList', type: 'ParsedObjectPropertyInfo[]'})
      .parameter({name: 'elementPropertyConstraints', type: 'ElementPropertyConstraint[]'})
      .parameter({name: 'aggregateContext', type: 'AggregateContext'})
    ...
    // // for (const element in materialProperties) {
    ...
    forScope
      .line('const propValue = object[propKey];');
    forScope
      .if('propValue === undefined || propValue === null')
        .line('parsingErrors.push(new ParsingError(')
        .line(`'dtmi:dtdl:parsingError:propertyValueNull',`)
        .line('{')
        .line(`cause: \`{primaryId:p} property '{property}' has value null, which is not allowed in DTDL models.\`,`)
        .line(`action: \`Change the value of '{property}' to a value that is legal for this property.\`,`)
    ...
    materialProperties.forEach((property) => {
      property.addCaseToParseSwitch(dtdlVersion, obverseClass, switchScope, !classIsAbstract, classIsPartition, 'valueCount', 'definedIn');
    });
    ...
    forScope
      ...
      .else()
        ...
        .line(`new ParsingError('dtmi:dtdl:parsingError:noTypeThatAllows',`)
        .line(`{`)
    ...
    MaterialClassAugmentor.addChecksForRequiredProperties(parsePropertiesMethod.body, !classIsAbstract);
  }

  private static _generateParseTokenMethod(obverseClass: TsClass, typeName:string, classIsBase:boolean, classIsAbstract:boolean) : void {
    const methodName = 'parseToken';
    const parseTokenMethod = obverseClass.method({name: methodName, returnType: 'number', abstract: false, isStatic: true})
      .parameter({name: 'model', type: 'Model'})
      .parameter({name: 'objectPropertyInfoList', type: 'ParsedObjectPropertyInfo[]'})
      .parameter({name: 'elementPropertyConstraints', type: 'ElementPropertyConstraint[]'})
      .parameter({name: 'valueConstraints', type: 'ValueConstraint[]'})
    ...
    typeTokenIfScope
      .if('parentId !== undefined')
        .line(`this.parseIdString(objectPropertyInfoList, elementPropertyConstraints, valueConstraints , aggregateContext, parsingErrors, token.toString(), parentId, propName, keyProp, allowedVersions);`)
        .line('valueCount++;');

    const isArrayElseIfScope = typeTokenIfScope
      .elseIf('Array.isArray(token)')
        .for('const elementToken of token')
          .line('valueCount += this.parseToken(model, objectPropertyInfoList, elementPropertyConstraints, valueConstraints, aggregateContext, parsingErrors, elementToken, parentId, definedIn, propName, dtmiSeg, keyProp, idRequired, typeRequired, allowIdReferenceSyntax, allowedVersions);');

    typeTokenIfScope
      .elseIf(`typeof token === 'object'`)
        .line('this.parseObject(model, objectPropertyInfoList, elementPropertyConstraints, valueConstraints, aggregateContext, parsingErrors, token, parentId, definedIn, propName, dtmiSeg, keyProp, idRequired, typeRequired, allowIdReferenceSyntax, allowedVersions);')
        .line('valueCount++;')
      .else()
        .line('parsingErrors.push(')
        .line(`new ParsingError('dtmi:dtdl:parsingError:badDtmiOrTerm',`)
        .line(`{`)
        .line(`cause: \`{primaryId:p} property '{property}' has value '{value}' that is not a DTMI or a DTDL term.\`,`)
        .line(`action: \`Replace the value of property '{property}, with a valid DTMI or a term defined by DTDL -- see https://github.com/Azure/opendigitaltwins-dtdl/tree/master/DTDL.\`,`)
        .line(`primaryId: parentId,`)
        .line(`property: propName,`)
  ...
  private static _generateAddConstraintMethod(obverseClass: TsClass, classIsAbstract:boolean, properties:MaterialProperty[]) {
    if (!classIsAbstract && properties.some((prop) => prop.propertyKind === PropertyKind.Object)) {
      const method = obverseClass.method({name: 'addConstraint', returnType: 'void', abstract: false, isStatic: false})
        .parameter({name: 'propertyName', type: 'string'})
      ...
      // method.body.line(`break;`).line(`}`);
      method.body.line(`}`);
    }
  }

  private static _generateAddInstancePropertyMethod(obverseClass: TsClass, classIsAbstract:boolean, properties:MaterialProperty[]) {
    if (!classIsAbstract && properties.some((prop) => prop.propertyKind === PropertyKind.Object)) {
      const method = obverseClass.method({name: 'addInstanceProperty', returnType: 'void', abstract: false, isStatic: false})
        .parameter({name: 'propertyName', type: 'string'})
  ...
  private static _generateSourceObjectProperty(obverseClass:TsClass, classIsBase:boolean) : void {
    ...
    //   const fType = 'any';
    //   // TODO: This has to be public as this is accessed from model parser class.
    //   obverseClass.field({name: `${fName}`, access: TsAccess.Public, type: `${fType}`});
    //   // obverseClass
    //   //   .getter({name: `${fName}`, access: TsAccess.Public, returnType: `${fType}`})
    //   //   .body
  ...
  private static _generateParseidStringMethod(obverseClass: TsClass, typeName: string, classIsBase:boolean, classIsAbstract:boolean) : void {
    const method = obverseClass.method({name: 'parseIdString', returnType: 'void', abstract: false, isStatic: true})
      .parameter({name: 'objectPropertyInfoList', type: 'ParsedObjectPropertyInfo[]'})
      .parameter({name: 'elementPropertyConstraints', type: 'ElementPropertyConstraint[]'})
      .parameter({name: 'valueConstraints', type: 'ValueConstraint[]'})
      .parameter({name: 'aggregateContext', type: 'AggregateContext'})
    ...
    ifScope
      .line('const objectPropertyInfo = {elementId: parentId, propertyName: propName ?? \'\', referencedElementId: elementId.value, keyProperty: keyProp, expectedKinds: this._concreteKinds[aggregateContext.dtdlVersion], allowedVersions: allowedVersions, badTypeCauseFormat: this._badTypeCauseFormat[aggregateContext.dtdlVersion], badTypeActionFormat: this._badTypeActionFormat[aggregateContext.dtdlVersion]};')
      .line('objectPropertyInfoList.push(objectPropertyInfo);')
        .if('valueConstraints != null && elementPropertyConstraints != null')
          .for('const vc of valueConstraints')
            .line('const elementPropertyConstraint = {parentId: parentId, propertyName: propName ?? \'\', elementId: elementId.value, valueConstraint: vc};')
            .line('elementPropertyConstraints.push(elementPropertyConstraint);');
    ifScope
      .else()
      .line('parsingErrors.push(')
