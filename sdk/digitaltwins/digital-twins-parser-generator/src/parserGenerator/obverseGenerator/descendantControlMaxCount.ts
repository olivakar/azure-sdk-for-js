...
export class DescendantControlMaxCount implements DescendantControl {
  ...
  constructor(dtdlVersion: number, rootClass: string, propertyNames: string[], isNarrow: boolean, maxCount: number) {
  ...
  addMembers(obverseClass: TsClass, typeName: string, classIsBase: boolean, classIsAbstract: boolean, materialProperties: MaterialProperty[]): void {
    ...
    if (classIsBase) {
      const baseClassMethod: TsFunction = obverseClass.method({name: this._methodName, returnType: 'number', abstract: true});
      baseClassMethod.summary(`Get the count of all descendant ${this._propertyNames.join(' or ')} properties.`);
      baseClassMethod.parameter({name: 'parsingErrors', type: 'ParsingError[]', description: `A list of ParsingErrors to which any parsing errors are added.`});
      ...
      obverseClass.field({name: statusFieldName, type: 'TraversalStatus', access: TsAccess.Protected});
      ...
      obverseClass.field({name: valueFieldName, type: 'number', access: TsAccess.Protected, value: 0});
      obverseClass.ctor.body.line(`this.${valueFieldName} = 0`);

      const concreteClassMethod: TsFunction = obverseClass.method({name: this._methodName, returnType: 'number'});
      ...
      const ifInProgress: TsIf = concreteClassMethod.body.if(`this.${statusFieldName} === TraversalStatus.InProgress`);
      ifInProgress
      ...
      for (const materialProperty of materialProperties) {
        ...
        if (materialProperty.propertyKind === PropertyKind.Object && (isRelevantProperty || !this._isNarrow)) {
          ...
          materialProperty.iterate(concreteClassMethod.body, varName)
            .line(`this.${valueFieldName} += ${varName.ref}.${this._methodName}(parsingErrors)${conditionalIncrement};`);
        }
      }
      ...
      const abstractClassMethod: TsFunction = obverseClass.method({name: this._methodName, returnType: 'number'});
      ...
      checkRestrictionsMethodBody
        ...
        .if(`num${this._coreName}Values > ${this._maxCount}`)
          .multiLine(`parsingErrors.push(new ParsingError(`)
            ...
            .line(`{`)
              .line(`cause: \`{primaryId:n} is at the root of a hierarchy that contains \${num${this._coreName}Values} ${this._propertiesDesc} properties, but the allowed maximum count is ${this._maxCount}.\`,`)
              .line(`action: \`Remove one or more ${this._propertiesDesc} property values to reduce the total count.\`,`)
              .line(`primaryId: this.${ParserGeneratorValues.IdentifierName},`)
            .line(`}));`);
    }
  }
