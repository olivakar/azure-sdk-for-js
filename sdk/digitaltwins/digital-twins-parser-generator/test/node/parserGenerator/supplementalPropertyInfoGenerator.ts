...
export class SupplementalPropertyInfoGenerator implements TypeGenerator {
  ...
  generateCode(parserLibrary: TsLibrary) {
    const infoInterface : TsInterface = parserLibrary.interface({name: 'SupplementalPropertyInfo', exports: true});
    infoInterface.import(`import {ValueConstraint} from '../parser';`);
    infoInterface.docString.line('Interface that provides information about a property that can be applied to a DTDL element that has a supplemental type.');
    ...
    const infoClass: TsClass = parserLibrary.class({name: 'SupplementalPropertyInfoImpl', exports: true, inheritance: [{name: 'SupplementalPropertyInfo', type: TsDeclarationType.Interface}]});
    ...
    infoClass.docString.line('Class that provides information about a property that can be applied to a DTDL element that has a supplemental type.');
    ...
    infoClass.ctor.body
      .inline('./src/parserPartial/supplementalPropertyInfoImpl.ts', 'constructor');
  }
}
