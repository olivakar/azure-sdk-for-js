...
export class BaseKindEnumGenerator implements TypeGenerator {
  ...
  generateType(parserLibrary: TsLibrary): void {
    Object.entries(this._materialClassesObject).forEach(
      ([key, materialClassDigest]) => {
        const typeKindValues = TypeKindValuesHelper.generateConcreteTypesKindValues(this._rawBaseType, key, materialClassDigest);
        ...
        if (typeKindValues.size > 0) {
          ...
          const params = {name: typeAliasNameForBaseKind, typeToBeAliased: typeKindValuesOred, exports: true};
