...
import {LiteralValidator} from '../parser/literalValidator';
...
export class EnumValueInfoImpl implements EnumValueInfo, TypeChecker {
  ...
  private _enumValueDatatype: string;
  ...
  constructor(dtdlVersion: number, id: string, childOf: string|undefined, definedIn: string|undefined, entityKind: EntityKind = EntityKind.ENUMVALUE) {
...
.add('dtmi:dtdl:class:NamedEntity');
    ...
    this._badTypeActionFormat[2] = `Provide a value for property '{property}' with @type EnumValue.`;
    this._badTypeActionFormat[3] = `Provide a value for property '{property}' with @type EnumValue.`;
    this._badTypeCauseFormat[2] = `{primaryId:p} property '{property}' has value{secondaryId:e} that does not have @type of EnumValue.`;
    this._badTypeCauseFormat[3] = `{primaryId:p} property '{property}' has value{secondaryId:e} that does not have @type of EnumValue.`;
  }
  ...
  doesHaveType(typeId: string): boolean {
    return EnumValueInfoImpl._versionlessTypes.has(new InDTMI(typeId).versionless) ||
this.supplementalTypes.some((st) => (st as SupplementalTypeInfoImpl).doesHaveType(typeId))
  ...
  static parseObject(model: Model, objectPropertyInfoList: ParsedObjectPropertyInfo[], elementPropertyConstraints: ElementPropertyConstraint[], valueConstraints: ValueConstraint[], aggregateContext: AggregateContext, parsingErrors: ParsingError[], object: {[index: string]: any}, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, dtmiSeg: string|undefined, keyProp: string|undefined, idRequired: boolean, typeRequired: boolean, allowIdReferenceSyntax: boolean, allowedVersions: Set<number>): any {
    // This is a method to parse the object read from DTDL into a type of EnumValueInfo
    ...
    (elementInfo as EnumValueInfoImpl).sourceObject = object;
    switch (childAggregateContext.dtdlVersion) {
    case 2: {
      (elementInfo as EnumValueInfoImpl)?.parsePropertiesV2(model, objectPropertyInfoList, elementPropertyConstraints, childAggregateContext, parsingErrors, object, definedIn, allowIdReferenceSyntax);
      break;
    }

    case 3: {
      (elementInfo as EnumValueInfoImpl)?.parsePropertiesV3(model, objectPropertyInfoList, elementPropertyConstraints, childAggregateContext, parsingErrors, object, definedIn, allowIdReferenceSyntax);
      break;
    }
    }
  ...
  static parseTypeArray(tokenArr: any[], elementId: string, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, aggregateContext: AggregateContext, parsingErrors: ParsingError[]): EnumValueInfo|undefined {
    const materialKinds : EntityKind[] = [];
    const elementInfo : Reference<EnumValueInfo> = referenceInit();
    ...
    for (const supplementalTypeId of supplementalTypeIds) {
      ...
      if (elementInfo.ref !== undefined && elementInfo.ref.entityKind !== undefined) {
        if (!(supplementalTypeInfo as SupplementalTypeInfoImpl)?.allowedCotypeKinds.includes(elementInfo.ref.entityKind)) {
          ...
          (elementInfo.ref as EnumValueInfoImpl).addType(supplementalTypeId, supplementalTypeInfo);
        }
      }
    }
  ...
  static tryParseTypeStringV2(typestring: string, elementId: string, parentId: string|undefined, definedIn: string|undefined, propName: string|undefined, materialKinds: EntityKind[], supplementalTypeIds: string[], elementInfo: Reference<EnumValueInfo>, undefinedTypes: string[], aggregateContext: AggregateContext, parsingErrors: ParsingError[]): boolean {
    switch (typestring) {
    case 'EnumValue':
  ...
  parsePropertiesV2(model: Model, objectPropertyInfoList: ParsedObjectPropertyInfo[], elementPropertyConstraints: ElementPropertyConstraint[], aggregateContext: AggregateContext, parsingErrors: ParsingError[], object: {[index: string]: any}, definedIn: string|undefined, allowIdReferenceSyntax: boolean): void {
    ...
    for (const propKey in object) {
      ...
      case 'dtmi:dtdl:property:displayName;2':
        ...
        continue;
      case 'enumValue':
      case 'dtmi:dtdl:property:enumValue;2':
        enumValuePropertyMissing = false;
        // eslint-disable-next-line no-case-declarations
        const enumValueValueAndType = ValueParser.parseSingularLiteralToken(this.id, 'enumValue', propValue, parsingErrors);
        this.enumValue = enumValueValueAndType.value;
        this._enumValueDatatype = enumValueValueAndType.typeFragment;
        continue;
      case 'name':
    ...
    if (enumValuePropertyMissing) {
      parsingErrors.push(new ParsingError(
        'dtmi:dtdl:parsingError:missingRequiredProperty',
        {
          cause: '{primaryId:p} property enumValue is required but missing.',
          action: 'Add a enumValue property to the object.',
          primaryId: this.id,
          property: 'enumValue',
        }));
    }
  ...
  parsePropertiesV3(model: Model, objectPropertyInfoList: ParsedObjectPropertyInfo[], elementPropertyConstraints: ElementPropertyConstraint[], aggregateContext: AggregateContext, parsingErrors: ParsingError[], object: {[index: string]: any}, definedIn: string|undefined, allowIdReferenceSyntax: boolean): void {
    ...
    for (const propKey in object) {
      ...
      case 'dtmi:dtdl:property:displayName;3':
        ...
        continue;
      case 'enumValue':
      case 'dtmi:dtdl:property:enumValue;3':
        enumValuePropertyMissing = false;
        // eslint-disable-next-line no-case-declarations
        const enumValueValueAndType = ValueParser.parseSingularLiteralToken(this.id, 'enumValue', propValue, parsingErrors);
        this.enumValue = enumValueValueAndType.value;
  ...
  validateInstanceV2(instanceElt: any, instanceName: string|undefined): boolean {
    if (!LiteralValidator.hasValue(instanceElt, this.enumValue)) {
      return false;
    }
...
**/
  setPartitionInfo(partitionJsonText: string): void {
    throw new Error(`attempt to set partition info on non-partition type EnumValueInfoInfo`);
  }
...
**/
  ...
  checkDescendantEnumValueDataType(ancestorId: InDTMI, datatype: string, parsingErrors: ParsingError[]): void {
    if (this._checkedDescendantEnumValueDatatype !== datatype) {
      ...
      if (typeof(this.enumValue) !== datatype) {
        parsingErrors.push(new ParsingError(
          'dtmi:dtdl:parsingError:nonConformantDatatype',
          {
            cause: `{primaryId:p} property 'enumValue' has value '{value}'. However, {secondaryId:n} specifies that the datatype of all descendant 'enumValue' propeties must be ${datatype}.`,
            action: `Change the value of property 'enumValue' to a value whose datatype is ${datatype}.`,
            primaryId: this.id,
            secondaryId: ancestorId.value,
            property: 'enumValue',
