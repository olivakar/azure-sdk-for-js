...
function main() {
  ...
  // Add imports
  tsClass
    .import('import {WindInterface} from \'./example_interface\';');
  ...
  tsClass
    ...
    .body
      .line('this._units = units;')
      .line('this._windPower = wind;')
      .line(`this._name = 'test'`);

  // Create getter
  tsClass.getter({name: 'name', returnType: 'string'}).body
    .line('return this._name');

  tsClass.getter({name: 'windSpeed', returnType: 'number | undefined'}).body
    .line('return this._windSpeed');

  tsClass.getter({name: 'units', returnType: 'string'}).body
    .line('return this._units');

  tsClass.getter({name: 'windPower', returnType: 'number'}).body
    .line('return this._windPower');

  // Create setter
  tsClass.setter({name: 'name'})
    .parameter({name: 'input', type: 'string'})
    .body
      .line('this._name = input;');

  const windSpeedMethod = tsClass
    .method({name: 'windSpeedMethod', returnType: 'void'})
    .parameter({name: 'input', type: 'number', optional: true})
    .body;
  ...
  tsClass.inline('./samples/typescript/codeGenerator/test.ts', 'codeblock1');
