...
export class DescendantControlExcludeType implements DescendantControl {
  ...
  constructor(dtdlVersion: number, rootClass: string, propertyNames: string[], isNarrow: boolean, excludeType: string, kindEnum: string, kindProperty: string) {
    ...
    this._excludeKind = `${kindEnum}.${NameFormatter.formatNameAsEnumValue(excludeType)}`;

    const propertyNameDisjunction = this._propertyNames.map((p) => NameFormatter.formatNameAsProperty(p)).join('Or');
    this._coreName = `${propertyNameDisjunction}${this._excludeType}${this._isNarrow ? 'Narrow' : ''}`;
  ...
  addMembers(obverseClass: TsClass, typeName: string, classIsBase: boolean, classIsAbstract: boolean, materialProperties: MaterialProperty[]): void {
    ...
    //   baseClassMethod.parameter({name: 'elementId', type: `Reference<${ParserGeneratorValues.IdentifierType}>`});
    ...
    obverseClass.field({name: idOfFieldName, type: `${ParserGeneratorValues.IdentifierType} | undefined`, access: TsAccess.Protected});
    obverseClass.ctor.body.line(`this.${idOfFieldName} = undefined;`);

    const concreteClassMethod = obverseClass.method({name: this._methodName, returnType: 'boolean'});
    concreteClassMethod.parameter({name: 'elementId', type: `Reference<${ParserGeneratorValues.IdentifierType}>`});
    ...
    for (const materialProperty of materialProperties) {
      ...
      if (materialProperty.propertyKind === PropertyKind.Object && (isRelevantProperty || !this._isNarrow)) {
        ...
        if (isRelevantProperty) {
          iterationScope.if(`${varName.ref}.${this._kindProperty} === ${this._excludeKind}`)
              .line(`elementId.ref = new InDTMI(${varName.ref}.${ParserGeneratorValues.IdentifierName});`)
              .line(`this.${idOfFieldName} = elementId.ref;`)
              .line(`return true;`);
        }

        iterationScope.if(`${varName.ref}.${this._methodName}(elementId)`)
            .line(`this.${idOfFieldName} = elementId.ref;`)
            .line('return true;');
      }
    }
  ...
  addRestriction(checkRestrictionsMethodBody: TsScope, dtdlVersion: number, typeName: string): void {
    if (this._dtdlVersion == dtdlVersion && this._rootClass === typeName) {
      ...
      checkRestrictionsMethodBody.line(`const ${elementIdName}: Reference<${ParserGeneratorValues.IdentifierType}> = {ref: undefined};`);
      checkRestrictionsMethodBody.line(`this.${this._methodName}(${elementIdName})`);
      checkRestrictionsMethodBody.if(`${elementIdName}.ref !== undefined`)
        .line('parsingErrors.push(new ParsingError(')
        .line(`'dtmi:dtdl:parsingError:excludedType',`)
        .line('{')
        .line(`cause: \`{primaryId:n} contains ${this._excludeType}{secondaryId:e}, which is not allowed in ${propertiesDesc} properties under elements of type {typeName}.\`,`)
        .line(`action: \`Remove the elements of type ${this._excludeType} from ${propertiesDesc} properties under the element of type ${typeName}.\`,`)
